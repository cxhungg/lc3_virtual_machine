...
;; Data section - placed early to keep LEA offsets in range
...
CHAR_S:         .FILL #83
CHAR_D:         .FILL #68
LOWER_A:        .FILL #97       ; ASCII 'a'
LOWER_Z:        .FILL #122      ; ASCII 'z'
CASE_DIFF:      .FILL #32       ; Difference between upper and lower case
...
DISPLAY_BOARD:
    ...
    LD R0, CLEAR_SCREEN_ADDR
    PUTS
    LD R0, HEADER_ADDR
    PUTS
    
    ;; Print board (4x4 grid)
    LD R1, BOARD_ADDR       ; Board pointer
...
INPUT_LOOP:
    ...
    LD R1, LOWER_A          ; Load 'a' (97)
    NOT R1, R1
    ADD R1, R1, #1
    ADD R1, R0, R1          ; Compare with 'a'
    BRn CHECK_DIRECTION     ; If < 'a', not lowercase
    
    LD R1, LOWER_Z          ; Load 'z' (122)
    NOT R1, R1
    ADD R1, R1, #1
    ADD R1, R0, R1          ; Compare with 'z'
    BRp CHECK_DIRECTION     ; If > 'z', not lowercase
    
    LD R1, CASE_DIFF        ; Load 32
    NOT R1, R1
    ADD R1, R1, #1
    ADD R0, R0, R1          ; Convert to uppercase (subtract 32)
...
FIND_EMPTY:
    ADD R3, R0, R1          ; Calculate address (base + offset)
    LDR R3, R3, #0          ; Load tile at that address
    ADD R3, R3, #0          ; Check if empty
    BRz PLACE_TILE
...
PLACE_TILE:
    ...
    ADD R4, R0, R1          ; Calculate address (base + offset)
    STR R3, R4, #0          ; Store tile at that address
...
;; Address pointers for strings (to avoid LEA range issues)
BOARD_ADDR:         .FILL BOARD
CLEAR_SCREEN_ADDR:  .FILL CLEAR_SCREEN
HEADER_ADDR:        .FILL HEADER
ROW_SEP_ADDR:       .FILL ROW_SEPARATOR
PIPE_ADDR:          .FILL PIPE_CHAR
EMPTY_TILE_ADDR:    .FILL EMPTY_TILE
SPACE_ADDR:         .FILL SPACE_CHAR
CONTROLS_ADDR:      .FILL CONTROLS
INPUT_PROMPT_ADDR:  .FILL INPUT_PROMPT
INVALID_INPUT_ADDR: .FILL INVALID_INPUT
NUM_2_ADDR:         .FILL NUM_2
NUM_4_ADDR:         .FILL NUM_4
NUM_8_ADDR:         .FILL NUM_8
NUM_16_ADDR:        .FILL NUM_16

;; String messages (placed at end)
